...
//
// KNOWN ISSUE: `compressRange` hangs (rc 124) on this engine — the
// nested `while (i+1 < nums.length && nums[i+1] === end + 1)` body that
// re-reads `nums[i+1]` after `i++` triggers the same register-reuse bug.
...
assert(false, "compressRange: known engine hang (rc 124) — see FAILURES.md");
