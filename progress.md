...
|---|---|---|
| 199 | Strict-only Phase A (compile-time errors): forced `is_strict = true` in compiler init (`context.c3`), unconditionalized 46 destructuring guards in `destructuring.c3`, octal literal/escape rejects in `expressions.c3`, restricted-name checks in `functions.c3`, duplicate property keys in `expressions.c3`, and remaining guards in `statements.c3`. Replaced `with_statement` parser with `case TokenType.WITH: COMPILE_ERROR`. Made `"use strict"` directive a no-op. Added duplicate-parameter-name rejection (new code) in `parse_function_body`/`compile_inner_function`/`compile_arrow_inner` via `check_dup_param` helper. Added unqualified `delete <identifier>` rejection (new code) in `expressions.c3:657`. Rosetta: 42-43/44 (apply_call+prototypes pre-existing failures, unchanged). Phase 0-1: 749→733 (–16 compile errors for sloppy code), Phase 2: 1150→1149 (–1). | –17 (Phase 0-1, 2) |
