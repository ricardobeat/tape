...
- [ ] **Module duplicate-lexical-name static semantics** — `var x; function x(){}` (and var/lexical clashes generally) must be a compile-phase SyntaxError in module code only; scripts allow it. 2 phase-0 fails: `dynamic-import/catch/nested-async-gen-{await,return-await}-eval-script-code-target.js`. Needs a module-scoped decl-conflict check in the compiler.
...
- [ ] **`Object.seal` on non-fixed-length TypedArrays** — same [[PreventExtensions]] gap Object.freeze had (fixed s293 with ta_is_fixed_length); no test262 test names it today; apply the same helper when touched next.
