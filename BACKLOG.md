...
- [ ] **Follow-up — implicit-global crash**: `x = 1` at global scope (no `var`/`let`/`const`) raises `VM_ERROR` instead of creating a global (in strict mode this must be `ReferenceError`). Pre-existing in baseline (crashes in both modes); discovered during strict-mode investigation. Repro: `x = 1; console.log(x);` → `VM_ERROR: undefined`. Likely in PUTVAR for unresolved env-chain reference; trace to confirm whether the crash is in env resolution, refcounting, or TVal init.
