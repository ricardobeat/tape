...
- [ ] **`new Set(string)` / `new Map(string)`-family constructor gap (session 292, Set-methods agent):** `coll_construct` checks is-object before the iterable protocol, so a bare string iterable throws "argument is not an object" instead of iterating code points. Only visible via excluded staging tests today.
