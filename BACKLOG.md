...
- [ ] **`yield <operand>` inside a for-await dstr default fails to compile** — plain `yield` works (suite-covered); the operand form (`[x = yield "v"]`) SyntaxErrors whole-file. Reparse/scanner edge, pre-existing.
