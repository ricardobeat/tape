...
- [ ] **A warmed variable IC serves a stale value after `eval` redeclares the global** — `var g1=1; function r(){return g1}; r();r();r(); eval("var g1 = 2;"); r()` returns `1`; node returns `2`. Confirmed on unmodified `main`, so it predates the session-304 IC work. `eval`'s `var` redeclaration writes the binding without invalidating read sites already warmed against the old shape. Invisible to the whole gate: 49,814 test262 tests, rosetta and the local suite are all green with it present. Found while auditing the var IC guard chain

- [ ] **`test262_runner --worker` segfaults on `staging/sm/class/superPropProxies.js`** — exit 139, reproducible on clean `main` with that one path as the whole input, so it needs no preceding test and is not batch-state dependent. The same source runs clean under `./out/duktape_c3` with the harness concatenated, which puts the fault on the runner path rather than the language. Invisible to the gate because `staging` is skip-listed, so this is out of the targeted subset; it is recorded because a segfault in a shipped entry point is worth understanding regardless of which corpus surfaced it. The test combines `class mid extends new Proxy(base, handler)` with `super` property get/set through a proxy on the prototype chain; neither construct crashes on its own
