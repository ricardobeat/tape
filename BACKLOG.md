...
- [>] **`return` inside `finally` doesn't abort the rest of the finally block** — side effects after it wrongly execute (value is right); all function kinds; RET IN_FINALLY continues instead of jumping to ENDFINALLY. Opus agent running.
