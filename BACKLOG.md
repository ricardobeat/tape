...
- [ ] **Dynamic-function parameter yield/await validation** — `new GeneratorFunction('yield', '')`-style param strings aren't early-error-checked (shared Generator/AsyncGenerator ctor gap; `instance-yield-expr-in-param` tests).
...
- [ ] **AsyncGenerator `.return()` on suspendedStart awaiting promise** — 2 tests (`return-suspendedStart-promise/broken-promise`), pre-existing, distinct from the landed AwaitReturn work.
