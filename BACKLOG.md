...
- [x] **Plain argument AFTER a large spread still miscompiles** — fixed by raising the post-`SPREAD_ARG` `next_reg` floor to `SPREAD_MAX_SLOTS` (256) so the trailing arg's evaluation register lands above the worst-case spread window. Real code rarely passes 200+ element spreads; pathological cases still need argument-array lowering per the plan.
