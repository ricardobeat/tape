...
| [044-statement-destructure-iterator.md](044-statement-destructure-iterator.md) | 📋 PLANNED | Statement-level `var [h, ...t] = x` / array-destructuring-assignment still read by index/`.slice()`, broken for non-array iterables (generators, custom iterables) — the plan-042 iterator-protocol class on the statement path. Reroute `array_destructure`/`array_destructure_assign` through the existing `emit_destruct_bindings` helper |
