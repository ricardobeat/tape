...
| [047-descriptor-validation-matrix.md](047-descriptor-validation-matrix.md) | 📋 PLANNED | B49 wave: shared ToPropertyDescriptor + ValidateAndApplyPropertyDescriptor spec-op helpers, routed through defineProperty/defineProperties/create; ~276 direct fails + adjacent families; folds in plan 022 remainder |
| [049-arraybuffer-typedarray-dataview.md](049-arraybuffer-typedarray-dataview.md) | 📋 PLANNED | ArrayBuffer + 9 TypedArrays + DataView from the current facade stubs: HObjectExtra buffer/view structs (Duktape byte-length+shift layout, QuickJS detach semantics), element fast paths with convert-then-recheck, prototype-method reuse via ta_validate, $262.detachArrayBuffer host hook, new test262 phase 22 (+~2,460 executable tests) |
| [048-destructuring-completion.md](048-destructuring-completion.md) | 📋 PLANNED | Destructuring completion wave: 1,273 dstr failures (session-268 baseline, 81.9%) split into six verified buckets — NamedEvaluation defaults (~280), non-identifier pattern keys (~200 CE), IteratorClose (~170), nullish-source TypeError (~90), member/paren assignment targets (~120), for-in-head patterns; est. +1,000–1,200 tests |
