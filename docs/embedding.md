...
| `include/jse.h` | (source) | n/a | Hand-written C99 public header. |
...
- `re_exec` collided with glibc. The vendored regexp wrapper exported a function
  ...
  `LD_PRELOAD=out/libjse.so`. It is now named `re_run`. macOS's two-level
...
| `jse_last_error(rt)` | `const char *` | Message for the most recent failure. Never `NULL`; empty when none. Owned by the runtime, so copy it. Formatted without re-entering the VM. |
...
The callback receives an opaque `jse_call_ctx` and the `udata` pointer given at
registration, passed through untouched. Read arguments with `jse_argc` and
`jse_arg`; an index past the end yields a handle to `undefined` rather than an
error, matching JS. `jse_this`, `jse_new_target`, and `jse_is_construct` cover
method and constructor calls.

To return a value, `jse_return` takes a handle, while `jse_return_number`,
...
`jse_throw(ctx, handle)` *record* a throw and return normally, and the callback
...
`jse_call(ctx, func, argv, argc, this_val, out_val)` invokes a JS function from
inside a callback. On `JSE_OK`, `*out_val` is a runtime-owned handle you must
free. If the callee throws, the exception is recorded on your context and
`JSE_ERR_THROW` is returned; return promptly and let the engine propagate it.
...
into VM registers. This matters because `jse_call` can grow and reallocate the
...
`jse_value_free` or `jse_close`. It survives garbage collection, because the
...
live handles, and exceeding it returns `JSE_ERR_FULL` (verified: the first
...
Host functions are supported through `jse_register_fn` and `jse_call`; see
...
There is no property access from the host, and no `jse_get_prop`. Objects are
opaque handles; read them from a JS callback and return a primitive, or
serialise with `JSON.stringify`.

`jse_call` is callback-only. It takes a `jse_call_ctx`, so it works from inside
a host function but not from `main`. Use `jse_eval` at the top level.
